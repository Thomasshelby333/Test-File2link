...
async def private_receive_handler(c: Client, m: Message):
   ...
   # lang = getattr(Language, "en")
    ...
    try:
        ...
        stream_link = "{}/{}/{}".format(Var.DOMAIN, log_msg.id, file_name) if Var.ON_HEROKU or Var.NO_PORT else \
            "{}/{}/{}".format(Var.DOMAIN,
                                    log_msg.id,
                                    file_name)
...
async def channel_receive_handler(bot, broadcast):
    ...
    try:
        ...
        stream_link = "{}/{}".format(Var.DOMAIN, log_msg.id) if Var.ON_HEROKU or Var.NO_PORT else \
            "{}/{}".format(Var.DOMAIN,
                                    log_msg.id)
        await log_msg.reply_text(
            text=f"**Cʜᴀɴɴᴇʟ Nᴀᴍᴇ:** `{broadcast.chat.title}`\n**Cʜᴀɴɴᴇʟ ID:** `{broadcast.chat.id}`\n**Rᴇǫᴜᴇsᴛ ᴜʀʟ:** https://t.me/{(await bot.get_me()).username}?start=KRBOTS_{str(log_msg.id)}",
